...
class AuthService:
    ...
    async def _get_user_data(self, user_id: str) -> Dict[str, Any]:
        ...
        """
        try:
            # Busca na tabela c_equipe com joins
            result = self.supabase.admin.table('c_equipe').select(
                """
                *,
                c_lojas!inner(
                    id,
            ...
            if not result.data:
                raise NotFoundException("Usuário não encontrado no sistema")
            
            user_data = result.data
            loja_data = user_data.get('c_lojas', {})
            empresa_data = loja_data.get('c_empresas', {}) if loja_data else {}
            ...
            return {
                ...
                'ativo': user_data.get('ativo', True),
                'funcao': user_data.get('funcao'),
                'loja_id': loja_data.get('id') if loja_data else None,
                'loja_nome': loja_data.get('nome') if loja_data else None,
                'empresa_id': empresa_data.get('id') if empresa_data else None,
                'empresa_nome': empresa_data.get('nome') if empresa_data else None,
            }
